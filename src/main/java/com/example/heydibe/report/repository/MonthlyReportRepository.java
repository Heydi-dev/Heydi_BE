...
public interface MonthlyReportRepository extends JpaRepository<MonthlyReport, Long> {
    ...
    @Query("select mr.reportYearMonth from MonthlyReport mr where mr.userId = :userId order by mr.reportYearMonth asc")
    List<String> findAvailableMonths(Long userId);

    @Query("select max(mr.reportYearMonth) from MonthlyReport mr where mr.userId = :userId")
